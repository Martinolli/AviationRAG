...
6. D.5 persisted-record mapping design is now a required gate before any real
   structured chunk migration or reset/rebuild work.
...
## Persisted ChunkRecord Mapping Design Gate

D.5 defines the planned `StructuredDocumentChunkCandidate` to
`PersistedChunkRecord` contract in:

```text
docs/PERSISTED_CHUNK_RECORD_MAPPING_DESIGN.md
docs/persisted_chunk_record_mapping.json
```

Real migration cannot begin until:

1. D.5 design is approved.
2. D.5b synthetic mapper is implemented in a separate future phase.
3. A controlled local sample-persistence package passes validation.
...
ingestion, production migration, embedding generation, Astra rebuild, FAISS
rebuild, or production retrieval activation.

The D.5 gate is design-only. No persisted records are generated by D.5, and no
runtime ingestion behavior changes.
...
| D.5b | Synthetic persisted `ChunkRecord` mapper and local package dry run. | Future synthetic/local package work only; no real corpus, embeddings, Astra, or FAISS. |
