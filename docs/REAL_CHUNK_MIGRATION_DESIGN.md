...
The D.5 gate is design-only. D.5b adds offline synthetic/local
`PersistedChunkRecord` scaffolding and ignored package outputs only. No runtime
ingestion behavior changes.

The D.5b synthetic package dry run passed on the formal D.4c fixture with six
accepted records, zero rejected candidates, zero warnings, zero issues, and
