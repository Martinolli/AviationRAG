...
PROCESSED_TEXT_DIR = PROCESSED_DIR / "ProcessedText"
PROCESSED_TEXT_EXPANDED_DIR = PROCESSED_DIR / "ProcessedTextExpanded"
...
def aviation_rag_manager():
    ...
    try:
    	...
    	logger.info(f"aviation_corpus.pkl updated with {len(all_documents)} total documents.")
    except Exception as e:
    	logger.error(f"Error saving aviation_corpus.pkl: {e}", exc_info=True)
    	raise
