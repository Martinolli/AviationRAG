...
function isChunkIdExists(chunkId) {
    if (existingEmbeddings.hasOwnProperty(chunkId)) {
        console.log(`⚠️ Skipping (duplicate): ${chunkId}`);
        return true;
    }
    return false;
}
...
async function processFile(filePath) {
    try {
        ...
        for (const chunk of document.chunks) {
            ...
            console.log(`Checking chunks for: ${document.filename}`);
            document.chunks.forEach(chunk => console.log(`Found chunk ID: ${chunk.chunk_id}`));
...
async function generateEmbeddings() {
    ...
    if (newEmbeddings.length > 0) {
        ...
        // Convert embeddings into the required list format
        let updatedEmbeddings = { ...existingEmbeddings };

        // Convert embeddings into the required list format
        let updatedEmbeddingsList = Object.values(updatedEmbeddings);

        // Save in the correct format
        fs.writeFileSync(outputPath, JSON.stringify(updatedEmbeddingsList, null, 2), "utf-8");
        console.log("✅ Updated embeddings file in correct format.");
        ...
        console.log("✅ No new embeddings were generated. All chunks are already processed.");
    }
}
