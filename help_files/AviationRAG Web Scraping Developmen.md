...
#### **Objective**
To integrate web scraping into the AviationRAG pipeline without disrupting the current document processing workflow. The goal is to extract aviation-related content from specific webpages, structure it into the same chunk format as DOCX documents, and merge both sources before generating embeddings.
